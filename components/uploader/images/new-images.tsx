...
const UploadNewImages: FC<{
  ...
  const onChange = async (imageList: ImageListType, addUpdateIndex: number[] | undefined) => {
    ...
    setUploadedImages(results.filter((x) => x.image).map((x) => x.image) as string[])
  }
          ...
          <div className="upload__image-wrapper">
            ...
            >
              <label htmlFor="file-upload" className="z-20 flex flex-col-reverse items-center justify-center w-full h-full cursor-pointer">
                <p className="text-xs  text-gray-500">File should be of format .png, .jpg, .jpeg or .webp</p>
                  ...
                  <CardBody>
                    {selectedImages?.includes(getImageLink(image)) ? (
                      <Button
                        ...
                        color="danger"
                        onClick={() => {
                          setSelectedImages([...selectedImages.filter((x) => x !== image)])
                        }}
                      >
                        Unselect Image
                      </Button>
