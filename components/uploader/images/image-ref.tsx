...
import { getImagePlacecs } from '@/lib/storage-operations'
import { Button, Chip, Popover, PopoverContent, PopoverTrigger, Spinner } from '@nextui-org/react'
...
const ImageRef: FunctionComponent<ImageRefProps> = ({ selectedImage }) => {
  ...
  return (
    <Popover showArrow offset={10} placement="bottom" backdrop={'blur'} isOpen={isOpen} onOpenChange={(open) => setIsOpen(open)}>
      <PopoverTrigger>
        <Button size={'sm'} className="my-2" variant={'bordered'} color="primary" startContent={<CiCircleInfo />}>
          Image references
        </Button>
      </PopoverTrigger>
        ...
        {(titleProps) => (
          <div className="px-1 py-2 w-full">
            <div className="flex justify-between items-center">
              <p className="text-medium font-bold text-foreground" {...titleProps}>
                Image Refrances
              </p>
            ...
            {isLoading ? (
              ...
              <div className="p-2 mt-4 space-y-6">
                ...
                {Array.from(data?.results ?? []).map(([category, items]) => (
                  <div key={category} className="grid justify-items-center space-y-2">
                    <Chip classNames={{ base: 'rounded-none' }} color="secondary">
                      {category}
                    </Chip>
                    <ul className="list-disc list-inside" dir="rtl">
                      {items.map((item: any, index: any) => (
                        <li key={index} className="text-ellipsis overflow-hidden line-clamp-1">
                          {item.name}
                        </li>
                      ))}
                    </ul>
