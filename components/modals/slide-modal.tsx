...
const SlideModal: FunctionComponent<SlideModalProps> = () => {
  ...
  const handleSubmitFeature = (formData: Slider) => {
    ...
    if (data) {
      newObject = {
        ...newObject,
        home: { ...newObject.home, sliders: [...(newObject.home?.sliders?.filter((x) => x.uuid !== data.uuid) ?? []), formData] },
      }
    } else {
      newObject = {
        ...newObject,
            ...
            <SingleImageForm formik={formik} field="image" maxNumber={1}>
              {values.image && (
                <div className="image-item  border rounded-xl relative dark:bg-white w-28 mt-5">
                  <img src={`${process.env.NEXT_PUBLIC_IMAGE_URL!}${values.image}`} alt="" className="w-full rounded-xl w-28" />
                  <ShcdnButton
                    type="button"
                    size={'icon'}
                    variant={'ghost'}
