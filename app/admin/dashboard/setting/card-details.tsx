...
const CardDetails: FunctionComponent<Slider> = ({ call_to_action, image, sub_title, title, uuid, call_to_action_link }) => {
  ...
  const handleDelete = () => {
    ...
    newObject = {
      ...
      home: {
        ...newObject.home,
        sliders: [...(newObject.home?.sliders?.filter((x) => x.uuid !== uuid) ?? [])],
      },
    }
        ...
        <div className="flex gap-1 justify-between items-center w-full pt-3 pb-1 px-4">
          <div className="flex">
            <Button
              ...
              radius="full"
              variant="light"
              onPress={() => modal.onOpenSlide({ call_to_action, image, sub_title, title, uuid, call_to_action_link })}
            >
              <Edit />
            </Button>
            <Button
