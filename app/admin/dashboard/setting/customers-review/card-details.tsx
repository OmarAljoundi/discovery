...
import { Faq, Review } from '@/types/custom'
...
const CardDetails: FunctionComponent<Review> = ({ description, title, uuid }) => {
  ...
  const handleDelete = () => {
    ...
    newObject = {
      ...
      customers_review: {
        ...
        review: [...(newObject.customers_review?.review?.filter((x) => x.uuid !== uuid) ?? [])],
      },
    }
  ...
  return (
    <Card className="pt-4">
      ...
      <CardBody className="overflow-visible py-2 border-t pb-4 text-right">{description}</CardBody>
        ...
        <div className="flex gap-1 justify-between items-center w-full pt-3 pb-1 px-4">
          <div className="flex">
            <Button
              ...
              radius="full"
              variant="light"
              onPress={() => modal.onOpenCustomerReview({ description, title, uuid })}
            >
              <Edit />
            </Button>
            <Button
