...
import { Avatar, Checkbox, Chip, Switch } from '@nextui-org/react'
import { toast } from 'sonner'
import { getTourTypes, updateTourStatus } from '@/lib/operations'
...
export const columns: ColumnDef<Tour>[] = [
  {
    ...
    cell: ({ row }) => {
      ...
      return <Avatar src={`${process.env.NEXT_PUBLIC_IMAGE_URL}${image}`} className="max-w-[6rem] truncate" />
    },
  ...
  },
  {
    ...
    filterFn: (row, id, value) => {
      //@ts-ignore
      return row.original.start_day?.filter((x) => (value as string[]).includes(x))?.length > 0 || false
    },
  },
  {
    accessorKey: 'price',
